...
from tekore import scope as spotify_scope
from tekore._auth.expiring.client import OAUTH_AUTHORIZE_URL
...
@api.get("/session")
async def return_session(request: Request):
    return JSONResponse(request.session)


@api.get("/authorize")
async def authorize_mock(redirect_uri: str, state: str):
    query = {"code": code, "state": state}
    return RedirectResponse(f"{redirect_uri}?{urlencode(query)}")


@api.post("/api/token")
async def api_token_mock():
    return JSONResponse(
        {
            "access_token": "123456",
            "token_type": "Bearer",
...
async def test_full_auth_sequence(
    ...
    login_resp = await auth_client.get(
        f"/login", params=login_params, allow_redirects=False
    )
